...
class EspecialidadModel extends Crud
{
  ...
  const TABLE 		= 'especialidad'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'especialidad_id'; // nombre del id de la tabla fisica
	...
	public function create(){
		try{
			...
			$sql = "INSERT INTO $this->table (nombre, activo) VALUES (?,?)";
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->activo));
			$this->pdo = null;
	...
	public function update(){
		try{
			$this->pdo = parent::conexion();
			$sql = "UPDATE $this->table SET nombre =?, activo = ? WHERE $this->idTableName = ?";
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->activo, $this->especialidad_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, especialidad_id as value  
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['especialidad_id']);
			}

...
class TipoCentroModel extends Crud
{
  ...
  const TABLE 		= 'tipo_centro'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'tipo_centro_id'; // nombre del id de la tabla fisica
	...
	public function update(){
		try{
			...
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->descripcion, $this->tipo_centro_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, tipo_centro_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['tipo_centro_id']);
			}

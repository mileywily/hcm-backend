...
class SolicitudReportModel extends Connection
{
			...
			$sql = "  SELECT distinct a.beneficiario_id,a.solicitud_id,
					 ...
					 b.apellidos as nombre_titular,
                      ...
                      en.nombre as ENTE_RESPONSABLE,
					   a.observacion as OBSERVACION_SOLICITUD
					  ...
					  LEFT JOIN ente en on a.ente_id = en.ente_id 
					  ";
			
            if (array_key_exists('solicitud_id',$data)) 
			{
               $sql.=" where a.solicitud_id= ?  and a.tipo_atencion_id = ?  and a.estado_solicitud_id in (1,4) ";
            }
			else 
			{
...
public function Rep_Solic_TriajeLab_By_Atencion($data){
        ...
        try
        {
			...
			$sql = "  SELECT distinct
					 ...
					 b.nombres as nombre_beneficiario,
					  ...
					  a.observaciones as OBSERVACION_ORDEN
	...
	public function Reporte_por_Solicitud_Servicio($data){
        try
        {
			...
			$sql = " SELECT distinct date_format(s.fecha_solicitud, '%d/%m/%Y') AS fecha_solicitud  ,
					 s.solicitud_id,t.descripcion as tipo_solicitud,es.descripcion as estado_solicitud,
					 ...
					 LEFT  JOIN causa c on c.causa_id = s.causa_solicitud_id
					 LEFT JOIN domicilio dom on dom.domicilio_id = s.domicilio_id
                                         LEFT JOIN ente en on s.ente_id = en.ente_id 

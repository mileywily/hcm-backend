...
class EnteModel extends Crud
{
  ...
  const TABLE 		= 'ente'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'ente_id'; // nombre del id de la tabla fisica
	...
	public function update(){
		try{
			...
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->descripcion, $this->ente_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, ente_id as value  
										FROM $this->table");
            $stm->execute();

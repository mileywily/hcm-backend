...
class SolicitudServicioModel extends Crud
{

  /* campos de la tabla */
  public  $solicitud_servicio_id;
  public  $solicitud_id;
  private $servicio_id;

  const TABLE 		= 'solicitud_servicio'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'solicitud_servicio_id'; // nombre del id de la tabla fisica
	...
	public function __construct($data)
	{
		...
		$this->solicitud_servicio_id	= $data["solicitud_servicio_id"];
	...
	public function create(){
		try{
			...
			if (is_countable($registros) &&  count($registros) >= 1){
				$this->response->setResponse(ListaCodigoMensaje::$ACCION_AGREGAR_REG, "El servicio ya se encuentra asociado a la solicitud. ", ListaCodigoMensaje::$COD_ERROR );
			}else{
	...
	public function getAllBySolicitud(){
		try
		{
			...
			$sql = "
				    ...
				    INNER JOIN servicio_tipoatencion st on ss.servicio_id = st.servicio_id and st.tipo_atencion_id =sol.tipo_atencion_id
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['solicitud_servicio_id']);
			}

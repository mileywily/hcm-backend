...
class EstadoAtencionModel extends Crud
{

  /* campos de la tabla */
  public  $estado_atencion_id;
  private $nombre;
  private $descripcion;

  const TABLE 		= 'estado_atencion'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'estado_atencion_id'; // nombre del id de la tabla fisica
	...
	public function update(){
		try{
			...
			$stm->execute(array($this->nombre, $this->descripcion, $this->estado_atencion_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, estado_atencion_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['estado_atencion_id']);
			}

...
class PrioridadModel extends Crud
{
  ...
  const TABLE 		= 'prioridad'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'prioridad_id'; // nombre del id de la tabla fisica
	...
	public function update(){
		try{
			...
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->descripcion, $this->prioridad_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, prioridad_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['prioridad_id']);
			}

...
class RecaudoModel extends Crud
{
  ...
  const TABLE 		= 'recaudo'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'recaudo_id'; // nombre del id de la tabla fisica
	...
	public function __construct($data)
	{
		parent::__construct(self::TABLE, self::IDNAMETABLE);
			...
			$this->recaudo_id     = isset($data["recaudo_id"])? $data["recaudo_id"]: null;
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, recaudo_id as value  
										FROM $this->table ");
            $stm->execute();

...
class TipoLaboratorioModel extends Crud
{

  /* campos de la tabla */
  public  $tipo_laboratorio_id;
  private $nombre;
  private $descripcion;

  const TABLE 		= 'tipo_laboratorio'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'tipo_laboratorio_id'; // nombre del id de la tabla fisica
	...
	public function __construct($data)
	{
		...
		$this->tipo_cargo_id	= $data["tipo_laboratorio_id"];
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, tipo_laboratorio_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['tipo_laboratorio_id']);
			}

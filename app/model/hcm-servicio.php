...
class ServicioModel extends Crud
{
  ...
  const TABLE 		= 'servicio'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'servicio_id'; // nombre del id de la tabla fisica
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, servicio_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['servicio_id']);
			}

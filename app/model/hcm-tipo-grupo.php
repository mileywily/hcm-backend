...
class TipoGrupoModel extends Crud
{
  ...
  const TABLE 		= 'tipo_grupo'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'tipo_grupo_id'; // nombre del id de la tabla fisica
	...
	public function __construct($data)
	{
		...
		$this->tipo_cargo_id	= $data["tipo_grupo_id"];
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, tipo_grupo_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['tipo_grupo_id']);
			}

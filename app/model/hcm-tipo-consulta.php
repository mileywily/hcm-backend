...
class TipoConsultaModel extends Crud
{
  ...
  const TABLE 		= 'tipo_consulta'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'tipo_consulta_id'; // nombre del id de la tabla fisica
	...
	public function update(){
		try{
			...
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->descripcion, $this->tipo_consulta_id));
	...
	public function getAllCombo(){
        ...
        {
            $this->pdo = parent::conexion();
            $stm = $this->pdo->prepare("SELECT *, nombre as label, tipo_consulta_id as value  
										FROM $this->table");
            $stm->execute();
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['tipo_consulta_id']);
			}

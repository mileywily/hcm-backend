...
class TipoCupoModel extends Crud
{
  ...
  const TABLE 		= 'tipo_cupo'; 	// Nombre de la tabla fisica
  const IDNAMETABLE = 'tipo_cupo_id'; // nombre del id de la tabla fisica
	...
	public function __construct($data)
	{
		...
		$this->tipo_cargo_id		= $data["tipo_cupo_id"];
	...
	public function update(){
		try{
			...
			$stm = $this->pdo->prepare($sql);
			$stm->execute(array($this->nombre, $this->descripcion, $this->tipo_cupo_id));
	...
	public function deleteByLote($data)
	{
		try
		{
			for ($i=0; $i < count($data) ; $i++) {
				$this->delete($data[$i]['tipo_cupo_id']);
			}
